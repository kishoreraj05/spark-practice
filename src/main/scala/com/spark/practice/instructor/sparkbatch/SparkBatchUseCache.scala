...
object SparkBatchUseCache {
  ...
  def main(args: Array[String]): Unit = {
    ...
    writeOutputInJson(resultdf1, "male_female_ratio_in_meghalaya")
    val resultdf2 = spark.sql("select * from resultDF where area = 'India' ")
    writeOutputInJson(resultdf2, "male_female_ratio_in_india")

    writeOutputInJsonWithPartitionCol(resultDf, "male_female_ratio_for_each_area", "area")
    resultDf.unpersist()
